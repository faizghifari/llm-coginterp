...
lives in `scripts/lib/` and is exposed through `manage_data.py` at the
repo root:

| Old one-off script(s) | Replaced by |
|---|---|
| `check_dupes.py`, `check_dupes2.py`, `analyze_dupes.py`, `analyze_source_trust.py` | `manage_data.py dupes` |
| `deduplicate_results.py`, `fix_results_issues.py` (dedup portion) | `manage_data.py dedup` |
| `find_aliases.py` | `manage_data.py find-aliases` |
| `fix_model_aliases.py`, `fix_results_fk.py`, `merge_models.py` | `manage_data.py apply-aliases --map-file <renames.json>` |
| `standardize_model_ids.py` (root) | `manage_data.py standardize-ids` |
| `analyze_models.py`, `categorize_models.py` | `manage_data.py categorize-models` |
