...
- [ ] 126 conflicting-evaluation groups remain, all from non-HF live leaderboards/sources (chat.lmsys.org Arena, vectara/hallucination-leaderboard, IBM mt-rag-benchmark, crux-eval, opencompass, lmarena/arena-hard-auto, and ~15 individual arXiv papers with 1-11 conflicts each). Same root-cause pattern as the HF case (a live, periodically-updated source scraped twice at different times) but each needs its own re-verification approach (some are clean APIs/CSVs, some are PDF papers that can't be "re-fetched live" at all) — scoped out of the 2026-06-16 pass pending a decision on whether to extend the same live-reverification treatment. Run `scripts/manage_data.py dupes --verbose` to see them.
