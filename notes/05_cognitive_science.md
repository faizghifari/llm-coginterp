# Cognitive Science

This document categorizes benchmarks related to cognitive science.

## Cognitive-Science Benchmarks

### Akata Games (2023) (`akata_games_2023`) [Link](https://arxiv.org/html/2502.14359v4)
**Subcategory:** theory-of-mind

No description available.

### Big Bench Hard (`bbh`) [Link](https://huggingface.co/datasets/Joschka/big_bench_hard)
**Subcategory:** reasoning

No description available.

### Binz & Schulz 2023 (`binz_schulz_2023`) [Link](https://arxiv.org/abs/2306.03917)
**Subcategory:** cognitive-modeling

No description available.

### CogBench (`cogbench`) [Link](https://arxiv.org/abs/2508.03360)
**Subcategory:** cognitive-modeling

No description available.

### ComplexBench (`complexbench`) [Link](https://github.com/thu-coai/complexbench)
**Subcategory:** reasoning

No description available.

### ELEPHANT (`elephant`) [Link](https://arxiv.org/abs/2505.13995)
**Subcategory:** social-behavior

No description available.

### EmoBench (`emobench`) [Link](https://arxiv.org/abs/2402.12071)
**Subcategory:** emotional-intelligence

No description available.

### EQ-Bench (`eqbench`) [Link](https://arxiv.org/abs/2312.06281)
**Subcategory:** emotional-intelligence

No description available.

### ExamsV (`exams_v`) [Link](https://aclanthology.org/2020.emnlp-main.438/)
**Subcategory:** knowledge

No description available.

### GPQA (`gpqa`) [Link](https://huggingface.co/datasets/Idavidrein/gpqa)
**Subcategory:** graduate-reasoning

No description available.

### GTBench (`gtbench`) [Link](https://arxiv.org/abs/2402.12348)
**Subcategory:** game-theory

No description available.

### LLM Chess (`llmchess`) [Link](https://arxiv.org/abs/2512.01992)
**Subcategory:** strategic-reasoning

No description available.

### LongShot (`longshot`) [Link](https://arxiv.org/html/2512.16978v1)
**Subcategory:** long-context

No description available.

### MMLU-Pro (`mmlu_prox`) [Link](https://huggingface.co/datasets/TIGER-Lab/MMLU-Pro)
**Subcategory:** knowledge

No description available.

### NeuroEval (`neuro_eval`) [Link](https://github.com/huggingface/evaluation-guidebook/blob/main/contents/automated-benchmarks/some-evaluation-datasets.md)
**Subcategory:** neuroscience

No description available.

### Pinocchio (`pinocchio`) [Link](https://arxiv.org/html/2310.05177v1)
**Subcategory:** lie-detection

No description available.

### RPGBench (`rpgbench`) [Link](https://arxiv.org/abs/2502.00595)
**Subcategory:** role-playing

No description available.

### SI-Bench (`sibench`) [Link](https://arxiv.org/abs/2510.23182)
**Subcategory:** social-intelligence

No description available.

### SOTOPIA (`sotopia`) [Link](https://arxiv.org/abs/2310.11667)
**Subcategory:** social-simulation

No description available.

### TemporalBench (`temporalbench`) [Link](https://arxiv.org/abs/2410.10818)
**Subcategory:** temporal-reasoning

No description available.

### Triangulating (`triangulating`) [Link](https://arxiv.org/abs/2502.14359)
**Subcategory:** reasoning

No description available.

### Ullman ToM (2023) (`ullman_tom_2023`) [Link](https://aclanthology.org/2023.findings-emnlp.717.pdf)
**Subcategory:** theory-of-mind

No description available.
